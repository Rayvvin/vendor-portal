...
const CustAvatar = (props) => {
  const { av_size, av_font_size, nw_record, field } = props;
  const record = useRecordContext();
  ...
  return (
    <Avatar
      {...stringAvatar(
        `${nw_record && field ? nw_record[field] : record[field]}`
      )}
    />
  );
};

const PickupRequestField = (props) => {
  const { source, currencies, currncy } = props;
  const record = useRecordContext();
  return (
    <Stack
    ...
    >
      ...
      <Stack>
        ...
        >
          ...
          <FunctionField
            // source="last_name"
            render={(record) => `Pickup Request #${record.id}`}
          />
        </Stack>
        ...
        >
          <Store sx={{ fontWeight: "400", fontSize: "14px" }} />
          <Stack direction="row" spacing={1} divider={<p>-</p>}>
            <ReferenceField
              ...
              link={false}
            >
              <ReferenceField reference="store" source="store_id" link={false}>
                <TextField source="name" />
              </ReferenceField>
            </ReferenceField>
            {/* <ReferenceField
...
const PickupRequestSummary = (props) => {
      ...
      comp: (
        <Stack marginTop={1}>
          <ReferenceManyCount
            ...
            filter={
              record && record.id
                ? { "order_id@in": `(${record.order_ids.join(",")})` }
                : null
            }
            // link
          />
  ...
  return (
    ...
    >
      ...
      >
        ...
        <PickupRequestField label="Pickup Request" />
...
export default function PickupRequestsShowComp(props) {
  ...
  const logistics_org_details_list = [
    ...
    },
    {
      ...
      child: (
        <ReferenceField
          label="Collection Station"
          reference="collection_stations"
          source="collection_station_id"
          link={false}
          sx={{
            fontSize: { md: "15px", sm: "13px", xs: "14px" },
            color: "#8d9498",
  ...
  return (
    <SimpleShowLayout sx={{ backgroundColor: "transparent", padding: "0px" }}>
      ...
      >
        ...
        {PickupOrderShowList(isSmall, record, theme, currncy)}
      </Stack>
    </SimpleShowLayout>
  );
}

function PickupOrderShowList(isSmall, record, theme, currncy) {
  return (
    <Stack
      className="card"
      spacing={3}
