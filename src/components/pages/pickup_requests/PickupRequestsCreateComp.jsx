...
export default function PickupRequestCreateComp(props) {
  const { type } = props;
  ...
  const identity = useGetIdentity();
  const [product, setPickupRequest] = React.useState(null);
  const medusa = new Medusa({
    maxRetries: 3,
    baseUrl: import.meta.env.VITE_MEDUSA_URL,
    apiKey: identity?.data?.medusa_user?.api_token,
        ...
        >
          ...
          >
            <Stack
              ...
              minWidth={"200px"}
            >
              <Typography className="input_title">Vendor</Typography>
              <TextInput
                fullWidth
                variant="outlined"
                source="vendor_id"
